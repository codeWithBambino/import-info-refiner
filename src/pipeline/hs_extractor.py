...
def extract_hs_code(dataframe: pd.DataFrame, raw_manifest_filename: str, hs_json_path=HS_FILE_PATH) -> pd.DataFrame:
    ...
    try:
        ...
        dataframe["Verified HS Codes"] = dataframe["HS Code List"].apply(lambda x: hs_code_verifier(x, hs_json_path))
        dataframe["Extracted HS Code"] = dataframe["Verified HS Codes"].apply(lambda x: x[0] if x else None)
